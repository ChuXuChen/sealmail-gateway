...
以下邮件认证能力暂时挂起，不作为阶段 1-5 的验收阻塞项：

- DKIM。
- SPF。
- DMARC。

挂起期间，阶段 1-5 的实现审查与修复不以 DKIM 签名、SPF 校验或 DMARC 策略处理是否完整作为缺口判断。现有相关入口可以保留为兼容代码或空路径，但不得为了处理这三项而扩大阶段 1-5 的变更范围。

恢复处理时应单独补充阶段计划，明确 DKIM/SPF/DMARC 的行为保护、配置归属、错误分类、审计记录和前端/API 收敛边界。
