...
export const auditLogApi = {
  list: (params: { page?: number; size?: number; category?: string; type?: string; success?: boolean }) =>
    unwrapApiResponse(apiClient.get<ApiResponse<PageResponse<AuditLog>>>('/api/v1/audit-logs', { params })),

  getByType: (type: string, params: { page?: number; size?: number }) =>
    unwrapApiResponse(apiClient.get<ApiResponse<PageResponse<AuditLog>>>(`/api/v1/audit-logs/type/${type}`, { params })),
  ...
  getByTimeRange: (startTime: string, endTime: string, params: { page?: number; size?: number }) =>
    unwrapApiResponse(apiClient.get<ApiResponse<PageResponse<AuditLog>>>('/api/v1/audit-logs/time-range', {
